...
// 注册 
	public function actionRegister()
	{
		if (IS_POST) {
			...
			$parent_idd = input('parent_idd', '', array('htmlspecialchars', 'trim')); //推荐人UID
			...
			if(get_affiliate()==0){ //非推荐注册时，判断推荐人是否存在			
			$uid = intval($parent_idd);										
		if ($GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['ecs']->table('users') . ('WHERE user_id = \'' . $uid . '\''))) {						
			cookie('ecshop_affiliate_uid', $uid);
		}else
		{			
			exit(json_encode(array('status' => 'n', 'info' => '推荐人邀请码不存在')));
		}						
			}
//
			if (empty($mobile)) {
 ...
 //  引用  mobile/app/Helpers/passport_helper.php  第4行 register （ 注册）   
			if (register($username, $password, $email, $other) !== false) {
        ...
        $parent_id=get_affiliate();  // 获取推荐人缓存
	...
	//分享注册	
	public function actionRegistershare()
	{
		if (IS_POST) {
			$username = input('mobile', '', array('htmlspecialchars', 'trim'));
		...
		//	$back_act=url("user/login/downAPP","",false,true);
...
//
 ...
 //  引用  mobile/app/Helpers/passport_helper.php  第4行 register （ 注册）   
            ...
            $url=url('user/login/downAPP');
            
			exit(json_encode(array('status' => 'y', 'info' => sprintf(L('register_success'), $username), 'url' => $url)));
