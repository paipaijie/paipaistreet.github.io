...
abstract class AliBaseStrategy implements BaseStrategy
{
    ...
    protected function sendReq(array $data, $method = 'GET')
    {
        ...
        if (! $flag) {
        	...
        	//exit;
            throw new PayException('444支付宝返回数据被篡改。请检查网络是否安全！');
...
*/
//throw new PayException($preStr);

//throw new PayException($this->config->signType);

       // throw new PayException($sign);
       //$rsa = new Rsa2Encrypt($this->config->rsaAliPubKey);
       
         // throw new PayException($this->config->rsaAliPubKey);  
        ...
        } else if ($this->config->signType === 'RSA2') {// 使用rsa2方式
